...
try:
    from config.proxy import PROXY_RETRY_MAX_ATTEMPTS as _PROXY_RETRY_MAX_ATTEMPTS
except Exception:
    _PROXY_RETRY_MAX_ATTEMPTS = 4
_FOLLOW_AUTH_MAX_ATTEMPTS = max(1, min(4, int(_PROXY_RETRY_MAX_ATTEMPTS or 4)))
    ...
    *,
    ...
    for attempt in range(1, _FOLLOW_AUTH_MAX_ATTEMPTS + 1):
        try:
            ...
            if (
                "/api/accounts/user/register" in final_url
                or "/create-account/password" in final_url
            ) and not allow_password_registration:
                raise RuntimeError(f"[步骤4] 落入密码注册路径，但当前未启用密码注册: {final_url}")
...
def get_create_account_page(session: BrowserSession, current_url: str = "") -> str:
    """进入密码注册节点，并确认会话没有倒退到无关页面。"""
    url = "https://auth.openai.com/create-account/password"
    referer = str(current_url or "https://auth.openai.com/email-verification")
    headers = session.get_auth_navigate_headers(referer=referer)
    headers["sec-fetch-site"] = "same-origin"

    logger.info("[步骤5] 选择密码注册，进入创建账号-密码页...")
    resp = session.get(url, headers=headers, allow_redirects=True)
    if resp.status_code >= 400:
        raise RuntimeError(
            f"密码注册页访问失败 status={resp.status_code}: {(resp.text or '')[:240]}"
        )
    final_url = str(getattr(resp, "url", "") or url)
    if "/create-account/password" not in final_url:
        raise RuntimeError(f"密码注册页落点异常，未进入 create-account/password: {final_url}")
    logger.info("[步骤5] 已进入创建账号-密码页: %s", final_url)
    return final_url
...
def register_user(
    ...
    logger.info("[步骤7] 提交密码注册请求: %s", email)
    resp = session.post(url, headers=headers, data=body)
    if resp.status_code != 200:
        ...
        raise RuntimeError(
            f"密码注册失败 status={resp.status_code}: {(resp.text or '')[:300]}"
        )
    ...
    try:
        ...
        raise RuntimeError(
            f"密码注册响应不是 JSON: {(resp.text or '')[:240]}"
        ) from exc
    if not isinstance(data, dict):
        raise RuntimeError(f"密码注册响应格式错误: {data!r}")
    ...
    otp_url = continue_url.lower()
    if page_type not in otp_page_types and not any(
        marker in otp_url
        for marker in ("email-otp", "email-verification", "email_otp")
    ):
        raise RuntimeError(
            "密码注册响应未进入邮箱验证码节点: "
            f"page_type={page_type or '空'}, continue_url={continue_url or '空'}"
        )
    if not continue_url:
        raise RuntimeError(
            f"密码注册响应缺少 continue_url，无法触发邮箱验证码: {data}"
        )

    logger.info(
        "[步骤7] 密码注册已通过: page_type=%s, has_continue_url=True",
        page_type or "未知",
    )
    return data


def follow_password_registration_continue(
    session: BrowserSession,
    register_result: dict,
) -> str:
    """只跟随一次密码注册返回的 continue_url，由服务端触发首封 OTP。"""
    page = register_result.get("page") if isinstance(register_result, dict) else {}
    page = page if isinstance(page, dict) else {}
    continue_url = str(
        register_result.get("continue_url")
        or register_result.get("external_url")
        or register_result.get("url")
        or page.get("continue_url")
        or page.get("external_url")
        or page.get("url")
        or ""
    ...
    if not continue_url:
        raise RuntimeError("密码注册响应缺少 continue_url")
    ...
    headers["sec-fetch-site"] = "same-origin"
    logger.info("[步骤8] 跟随密码注册 continue_url，触发邮箱验证码...")
    resp = session.get(continue_url, headers=headers, allow_redirects=True)
    if resp.status_code >= 400:
        raise RuntimeError(
            f"密码注册后触发邮箱验证码失败 status={resp.status_code}: "
            f"{(resp.text or '')[:240]}"
        )
    final_url = str(getattr(resp, "url", "") or continue_url)
    if not any(
        marker in final_url.lower()
        for marker in ("email-verification", "email-otp", "email_otp")
    ):
        raise RuntimeError(f"密码注册后未进入邮箱验证码页: {final_url}")
    logger.info("[步骤8] 邮箱验证码已触发，落点: %s", final_url)
    return final_url
