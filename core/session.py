...
class BrowserSession:
    ...
    def __init__(self, proxy: str = None, *, pre_proxy: str | None = None, detect_exit_geo: bool = True):
        """
        初始化会话。
        ...
        Args:
            proxy: 代理地址，如 "socks5h://user:pass@host:port"。
                   ...
                   显式传 "" 表示禁用代理。
            pre_proxy: 前置代理（uplink）。所有经本会话 proxy 的连接先走此前置代理再连目标代理，
                       用于绕过代理商的源 IP 白名单。None = 使用 config.PROXY_PRE_PROXY；
                       "" 显式禁用。libcurl 前置代理仅支持 socks5h:// / socks5://。
        ...
        # pre_proxy=None → 读取配置默认值；"" → 显式禁用
        if pre_proxy is None:
            try:
                from config.proxy import PROXY_PRE_PROXY as _pp
            except Exception:
                _pp = ""
            self.pre_proxy = str(_pp or "").strip()
        else:
            self.pre_proxy = str(pre_proxy or "").strip()
        ...
        # 前置代理（uplink）：curl_cffi 每次请求都会重置 curl 选项，
        # 因此必须写进 session.curl_options（set_curl_options 每次请求统一应用），
        # 而不能只 setopt 一次。只对 socks5 生效；http:// 会由 libcurl 报 unsupported。
        if self.proxy and self.pre_proxy:
            if self.pre_proxy.startswith(("socks", "socks5", "socks5h")):
                try:
                    co = getattr(self.session, "curl_options", None)
                    if co is None:
                        co = {}
                        self.session.curl_options = co
                    co[CurlOpt.PRE_PROXY] = self.pre_proxy
                except Exception as exc:
                    logger.warning("[代理] 注入前置代理失败，将直连目标代理: %s", exc)
            else:
                logger.warning("[代理] 前置代理 %s 不是 SOCKS 协议，libcurl 的 PRE_PROXY 不支持 HTTP，已忽略", self.pre_proxy)
