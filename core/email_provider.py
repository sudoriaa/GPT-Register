...
def wait_for_reset_link(
    ...
    """等待并返回该邮箱最新的 OpenAI 密码重置链接（完整 URL 字符串）。

    与 wait_for_otp 同构：按邮箱实际来源分发。item-based 来源复用
    fetch_latest_otp 的轮询/settle 状态机，把抽码器换成 extract_reset_link；
    generic_api/xbovo 走独立的 fetch_latest_reset_link。

    USE_EMAIL_SERVICE=False（manual 通道）读不了邮件正文，直接抛错——
    此时应设 BACKFILL_SET_PASSWORD=False 跳过设密码。
    """
    try:
        from config import email as _email_cfg
        use_service = bool(getattr(_email_cfg, "USE_EMAIL_SERVICE", True))
    ...
    if not use_service:
        raise RuntimeError(
            "manual 模式（USE_EMAIL_SERVICE=False）无法自动读取重置邮件链接；"
            "请使用自动邮箱源，或设置 BACKFILL_SET_PASSWORD=False 跳过设置密码"
        )
    ...
    from core.otp_utils import extract_reset_link as _extract_reset_link

    source = resolve_email_source(email)
    if source in ("generic_api", "xbovo"):
        from core.generic_api_mail_client import fetch_latest_reset_link
