...
from core.registration_ip import (
    detect_playwright_registration_geo,
...
# 密码提交/一次性验证码入口后的授权跳转可能包含 CF 处理和多次导航。
# 只有确认进入 OTP、资料页或登录态后才允许继续，避免过早调用 OTP 控件。
_PASSWORD_TRANSITION_TIMEOUT = 30
# OTP 提交后的 CF/SPA 转场与密码节点使用同一完整观察窗口；fast mode 也不能缩短
# 这个窗口，否则慢转场会被误判为未知成功并过早读取 session。
_OTP_TRANSITION_TIMEOUT = max(_PASSWORD_TRANSITION_TIMEOUT, 30)
_CLOUD_PROXY_PREFLIGHT_ATTEMPTS = 4
...
def _close_cloud_preflight_attempt(
    client,
    provider_prefix: str,
    browser,
    session_info,
    *,
    ...
    """回收一次未通过注册代理预检的云浏览器会话。"""
    _close_browser_use_session(browser, reason=reason)
    if provider_prefix != "skyvern" or session_info is None:
        return
    session_id = str(getattr(session_info, "session_id", "") or "").strip()
    if not session_id or not hasattr(client, "close_browser_session"):
        return
    try:
        client.close_browser_session(session_id)
        logger.info("[Skyvern] 已关闭未通过代理预检的 browser session：%s", session_id)
    except Exception as exc:
        logger.warning(
            "[Skyvern] 关闭未通过代理预检的 browser session 失败：%s: %s",
            type(exc).__name__,
            str(exc)[:180],
        )


def _open_cloud_session_with_proxy_preflight(
    playwright,
    client,
    *,
    provider_prefix: str,
    ...
    max_attempts: int = _CLOUD_PROXY_PREFLIGHT_ATTEMPTS,
):
    """在填写邮箱前创建并验证云浏览器会话，失败时换新会话。"""
    attempts = max(1, min(_CLOUD_PROXY_PREFLIGHT_ATTEMPTS, int(max_attempts or 1)))
    errors: list[str] = []
    ...
    for attempt in range(1, attempts + 1):
        ...
        try:
            ...
            page.set_default_navigation_timeout(
                _timeout_ms(getattr(_cfg, "BROWSER_USE_NAVIGATION_TIMEOUT", 90))
            )
            registration_geo = normalize_registration_geo(
                detect_playwright_registration_geo(
                    page,
                    log_prefix=f"[{cloud_label}注册][代理预检 {attempt}/{attempts}]",
                )
            )
            ...
            logger.info(
                "[%s][代理预检] 第 %s/%s 个会话通过：IP=%s country=%s",
                cloud_label,
                attempt,
                attempts,
                registration_geo["ip"],
            ...
            return session_info, browser, context, page, registration_geo
        except Exception as exc:
            if _is_manual_stop_exception(exc):
                _close_cloud_preflight_attempt(
                    client,
                    provider_prefix,
                    browser,
                    session_info,
            ...
            errors.append(f"{attempt}/{attempts} {detail}")
            logger.warning(
                "[%s][代理预检] 第 %s/%s 个会话失败，%s：%s",
                cloud_label,
                attempt,
                attempts,
                "准备更换新会话" if attempt < attempts else "已达到重试上限",
                detail,
            )
            _close_cloud_preflight_attempt(
                client,
                ...
                browser,
                session_info,
                reason=f"代理预检失败 {attempt}/{attempts}",
            )
            if attempt >= attempts:
                history = " | ".join(errors[-attempts:])
                raise RuntimeError(
                    f"{cloud_label} 注册代理/IP/国家预检连续 {attempts} 次失败，"
                    f"已停止且未提交邮箱；{history}"
                ) from exc

    raise RuntimeError(f"{cloud_label} 注册代理/IP/国家预检未完成")
...
def _check_manual_stop() -> None:
    try:
        ...
        if run_deadline_reached():
            raise RuntimeError(
                "注册超时（RUN_MAX_MINUTES 分钟），已停止当前浏览器恢复链并交给任务重试"
            )
    except ImportError:
        return
...
def _is_manual_stop_exception(exc: Exception) -> bool:
    return (
        ...
        or "手动停止" in str(exc)
        or "注册超时（RUN_MAX_MINUTES" in str(exc)
    )
...
def _fill_password_if_present(page, email: str, timeout: int = 25, context=None) -> str | None:
    ...
    while time.time() < end:
        ...
        if _click_passwordless_signup_if_present(page):
            ...
            while time.time() < wait_end:
                ...
                if state_after.get("state") == "chatgpt":
                    ...
                    return None
                if state_after.get("state") == "profile":
                    logger.info("[BrowserUse] 一次性验证码入口后已进入资料页")
                    return None
                time.sleep(0.2 if _fast_mode() else 0.5)
            raise RuntimeError(
                f"一次性验证码入口点击后仍未进入 OTP 页，禁止继续填写 OTP: state={_quick_auth_state(page)}"
            )
        if state == "login_password":
            raise RuntimeError(
                f"已进入登录密码页且未找到一次性验证码入口，按已注册/不可用邮箱处理并停用: url={state_info.get('url') or _page_url(page) or 'https://auth.openai.com/log-in/password'}"
            )
        ...
        if not _click_first(
            ...
            page.keyboard.press("Enter")
        _bu_delay("form")
        # 提交密码后等待真实页面状态，不能把仍在 password 页的页面交给 OTP 阶段。
        wait_end = time.time() + _PASSWORD_TRANSITION_TIMEOUT
        last_after = {}
        while time.time() < wait_end:
            page = _browser_use_heartbeat(page, context=context, label="password-submit-transition")
            last_after = _quick_auth_state(page)
            after_state = str(last_after.get("state") or "other")
            if after_state == "email_verification":
                logger.info("[BrowserUse] 密码提交后已进入邮箱验证码页")
                return password
            if after_state in ("chatgpt", "profile"):
                logger.info("[BrowserUse] 密码提交后已进入后续页面：state=%s", after_state)
                return password
            if after_state == "login_password":
                raise RuntimeError(
                    f"密码提交后进入登录密码页，按已注册/不可用邮箱处理并停用: url={last_after.get('url') or _page_url(page)}"
                )
            time.sleep(0.2 if _fast_mode() else 0.5)
        final_state = _quick_auth_state(page)
        if str(final_state.get("state") or "") == "password":
            raise RuntimeError(f"密码提交后仍停留在密码页，未进入 OTP 输入页: state={final_state}")
        raise RuntimeError(f"密码提交后未确认进入 OTP/后续页面，禁止继续填写 OTP: state={final_state or last_after}")
    return None
...
def _type_otp(page, code: str) -> None:
    ...
    state_info = _quick_auth_state(page)
    if str(state_info.get("state") or "") in ("password", "login_password"):
        raise RuntimeError(f"密码页尚未完成，未进入 OTP 输入阶段: state={state_info}")
...
def _wait_after_otp(page, timeout: int = _OTP_TRANSITION_TIMEOUT) -> str:
...
def _browser_registration_stage_state(page) -> str:
    """恢复链使用的一次只读节点探测。"""
    info = _quick_auth_state(page)
    state = str(info.get("state") or "other")
    if state != "other":
...
def _wait_for_otp_transition_or_raise(page) -> str:
    """等待 OTP 提交结果；未知转场必须进入阶段恢复链。"""
    outcome = _wait_after_otp(page, timeout=_OTP_TRANSITION_TIMEOUT)
    if outcome == "unknown":
        raise RuntimeError("OTP 提交后 30 秒仍未确认页面转场")
    return outcome


def _browser_recovery_actions(
    *,
    stage_url: str | None,
...
def _apply_browser_stage_recovery(
    page,
    context,
    stage: str,
    action: str,
    *,
    ...
    """执行一个 Playwright 恢复动作，返回可能切换后的 live page 与报告。"""
    _check_manual_stop()
    page = _pick_live_page(context, page) or page
    action_error = None
    nav_timeout = _timeout_ms(getattr(_cfg, "BROWSER_USE_NAVIGATION_TIMEOUT", 90))
    try:
        if action == "refresh":
            page.reload(wait_until="domcontentloaded", timeout=nav_timeout)
        elif action == "reenter_stage":
            page.goto(str(stage_url), wait_until="domcontentloaded", timeout=nav_timeout)
        elif action == "return_previous":
            try:
                page.go_back(wait_until="domcontentloaded", timeout=nav_timeout)
            except Exception as exc:
                logger.info("[BrowserUse][恢复][%s] go_back 未完成，改用上一步 URL：%s", stage, str(exc)[:160])
            page.goto(str(previous_url), wait_until="domcontentloaded", timeout=nav_timeout)
            replay_previous(page)
        elif action != "reprobe":
        ...
        logger.warning("[BrowserUse][恢复][%s] 动作 %s 未完成，继续重探测：%s", stage, action, action_error)

    _check_manual_stop()
    page = _pick_live_page(context, page) or page
    try:
        state = _browser_registration_stage_state(page)
    except Exception as exc:
        if _is_manual_stop_exception(exc):
            raise
        state = "unknown"
    ...
    report = {"action": action, "state": state, "url": _page_url(page)[:300]}
    if action_error:
        report["error"] = action_error
    logger.info("[BrowserUse][恢复][%s] %s", stage, report)
    return page, report


def _run_browser_stage_with_recovery(
    page,
    context,
    stage: str,
    operation,
    *,
    ...
    """执行 Browser Use 注册阶段，有限恢复全部失败后才抛错。"""
    actions = _browser_recovery_actions(
        stage_url=stage_url,
    ...
    for attempt in range(len(actions) + 1):
        ...
        page, report = _apply_browser_stage_recovery(
            page,
            context,
            stage,
            actions[attempt],
        ...
        if resume_from_state is not None:
            ...
            if done:
                logger.info(
                    "[BrowserUse][恢复][%s] 状态重探测确认已推进，跳过动作重放：state=%s",
                    stage, report.get("state"),
                )
                return result, page

    final_state = "unknown"
    try:
        final_state = _browser_registration_stage_state(page)
    except Exception:
        pass
    detail = f"state={final_state} url={_page_url(page)[:300]} history={str(history)[-1800:]}"
    raise RuntimeError(
...
def run_browser_use_registration(
    ...
    if provider in ("skyvern", "sv"):
        ...
        client = SkyvernClient(proxy_country_code=registration_country or None)
    else:
        cloud_label = "BrowserUse"
        provider_prefix = "browser_use"
        client = BrowserUseClient(proxy_country_code=registration_country or None)

    _set_log_provider_label(cloud_label)
    _t_all = _StepTimer(f"{cloud_label} 注册全流程")
    session_info_open = None
    ...
    logger.info(
        "[%s] 开始注册：%s targetCountry=%s local_proxy_arg=%s",
        cloud_label,
        email,
        registration_country or "不限",
    ...
    try:
        with sync_playwright() as p:
            _t_cdp = _StepTimer(f"连接 {cloud_label} CDP 并完成代理预检")
            session_info_open, browser, context, page, registration_geo = (
                _open_cloud_session_with_proxy_preflight(
                    p,
                    client,
            ...
            _t_cdp.done(
                f"ip={registration_geo['ip']} country={registration_geo['country'] or '?'}"
            )
            registration_ip = registration_geo["ip"]
            logger.info(
                "[%s] 已选定注册会话：proxyCountry=%s profileId=%s",
                cloud_label,
            ...
            _, page = _run_browser_stage_with_recovery(
                page,
                context,
                "打开注册入口",
                lambda active_page: active_page.goto(start_url, wait_until="domcontentloaded"),
                stage_url=start_url,
            ...
            next_state, page = _run_browser_stage_with_recovery(
                page,
                context,
                "提交邮箱",
                lambda active_page: _submit_email_until_transition(
                    active_page, context, email, attempts=2, timeout_ms=20000,
                ),
                stage_url=start_url,
                resume_from_state=lambda state, _report: (
                    state in ("password", "email_verification", "profile", "chatgpt"),
                    state,
                ),
            )
            _t_email.done()
            ...
            advanced_state = next_state if next_state in ("profile", "chatgpt") else None
            password_stage_url = _page_url(page)

            def _replay_email_before_password(active_page) -> None:
                _maybe_accept_cookies(active_page)
            ...
            try:
                if advanced_state is None:
                    def _password_resume(state, _report):
                        progressed = state in ("profile", "chatgpt") or (
                            next_state == "password" and state == "email_verification"
                        )
                        return progressed, (None, state if progressed else None)

                    password_result, page = _run_browser_stage_with_recovery(
                        page,
                        context,
                        "处理密码节点",
                        lambda active_page: (
                    ...
                    openai_password, recovered_state = password_result
                    if recovered_state in ("profile", "chatgpt"):
            ...
            for otp_attempt in otp_attempts:
                ...
                while time.time() < wait_end:
                    ...
                    if state == "password":
                        logger.warning("[BrowserUse][恢复][OTP] 页面迟到进入密码节点，先恢复密码步骤：state=%s", state_info)
                        recovered_password, page = _run_browser_stage_with_recovery(
                            page,
                            context,
                            "OTP 前补做密码节点",
                            lambda active_page: _fill_password_if_present(
                                active_page,
                                email,
                                timeout=10 if _fast_mode() else 20,
                                context=context,
                            ),
                            stage_url=_page_url(page),
                            previous_url=start_url,
                            replay_previous=_replay_email_before_password,
                            resume_from_state=lambda recovered_state, _report: (
                                recovered_state in ("email_verification", "profile", "chatgpt"),
                                None,
                            ),
                        )
                ...
                otp_stage_url = _page_url(page)

                def _prepare_otp_input(active_page):
                    nonlocal openai_password
                    state_before_otp = _browser_registration_stage_state(active_page)
                    if state_before_otp in ("password", "login_password"):
                        recovered_password = _fill_password_if_present(
                            active_page,
                            email,
                            timeout=10 if _fast_mode() else 20,
                            context=context,
                        ...
                        if recovered_password:
                            openai_password = recovered_password
                    _clear_otp_inputs(active_page)
                    _type_otp(active_page, current_otp)
                    return "typed"

                otp_prepare_state, page = _run_browser_stage_with_recovery(
                    page,
                    context,
                    "填写邮箱验证码",
                    _prepare_otp_input,
                    stage_url=otp_stage_url,
                    # OTP 不返回上一步，避免重新发码后继续消费旧验证码。
                    resume_from_state=lambda state, _report: (
                        state in ("profile", "chatgpt"),
                        state,
                    ),
                )
                if otp_prepare_state in ("profile", "chatgpt"):
                    advanced_state = otp_prepare_state
                    _t_otp_submit.done(f"state={otp_prepare_state}")
                    break
                _bu_delay("otp_input")
                try:
                ...
                outcome, page = _run_browser_stage_with_recovery(
                    page,
                    context,
                    "等待邮箱验证码提交结果",
                    _wait_for_otp_transition_or_raise,
                    stage_url=otp_stage_url,
                    # 这里只重探测提交结果，不会再次填写或提交同一个 OTP。
                    resume_from_state=lambda state, _report: (
                        state in ("profile", "chatgpt"),
                        "accepted",
                    ),
            ...
            profile_stage_url = _page_url(page)
            profile_submitted, page = _run_browser_stage_with_recovery(
                page,
                context,
                "完成资料节点",
                lambda active_page: _complete_profile_page(
                    active_page,
                    name,
                    birthday,
                    timeout=28 if _fast_mode() else 60,
                ),
            ...
            session_info, page = _run_browser_stage_with_recovery(
                page,
                context,
                "读取登录会话",
                lambda active_page: _fetch_chatgpt_session(
                    active_page,
                    context=context,
