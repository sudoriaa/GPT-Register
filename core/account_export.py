...
def _validate_reauth_otp(session: BrowserSession, code: str) -> str:
    ...
    if resp.status_code != 200:
        logger.error(f"[2FA] 重认证 OTP 验证请求失败, 状态码: {resp.status_code}")
        logger.error(f"[2FA] 响应内容: {resp.text}")
        ...
        if resp.status_code in (400, 401, 422) and any(k in low for k in (
            ...
            raise EmailOtpInvalidError(
                f"重认证邮箱验证码无效或已过期: status={resp.status_code}, body={(resp.text or '')[:240]}"
            )
        resp.raise_for_status()
    data = resp.json()
...
# auth.openai.com React SPA 的真实密码重置协议（经 JS bundle 逆向确认）：
#   START:  POST {base}/password/send-otp     → 发重置邮件
#           （intent=passwordless_login_send_otp 时 SPA 走 {base}/passwordless/send-otp，
#            那封是登录一次性码不是重置链接，本模块固定用 /password/send-otp 拿重置链接）
#   RESET:  POST {base}/password/reset         body 只有 {"password": <新密码>}
#           密码不随 body/query 传，靠访问重置链接时 set 的 cookie 认证（credentials include）
# base = https://auth.openai.com/api/accounts
_AUTH_API_BASE = "https://auth.openai.com/api/accounts"
_RESET_START_URL = _AUTH_API_BASE + "/password/send-otp"
_RESET_SUBMIT_URL = _AUTH_API_BASE + "/password/reset"
...
def trigger_password_reset(
    ...
    """
    发送 ChatGPT 密码重置邮件：POST /api/accounts/password/send-otp。

    逆向确认（_hit_route-ksOdhr-l.js）：body 只允许 {intent: "send_otp"}，
    账号取自会话 cookie，带 username/email 一律被拒 400（Unknown parameter）。
    email 参数仅用于日志。响应全文落日志供实弹校准；HTTP >= 400 抛 RuntimeError。
    """
    headers = session.get_auth_headers(referer="https://auth.openai.com/")
    headers["content-type"] = "application/json"
    body = json.dumps({"intent": "send_otp"})
    logger.info("[2FA] [PWD] send-otp 发送重置邮件: %s", email)
    resp = session.post(_RESET_START_URL, headers=headers, data=body)
    logger.info(
        "[2FA] [PWD] send-otp 响应: HTTP %s body=%s",
        resp.status_code, (resp.text or "")[:400],
    )
    if resp.status_code >= 400:
        raise RuntimeError(f"password send-otp 失败: HTTP {resp.status_code} {(resp.text or '')[:240]}")
    return True


def _parse_reset_token(reset_url: str) -> tuple[str, str]:
    """从重置链接解析 (origin, token)。支持 query 参数或 path 段两种形态。"""
    parsed = urlparse(reset_url)
    token = ""
    q = parse_qs(parsed.query, keep_blank_values=True)
    for key in ("token", "code", "reset_token", "resetcode", "confirmation_token", "t"):
        if q.get(key) and q[key][0]:
            token = q[key][0]
            break
    if not token:
        segs = [s for s in parsed.path.split("/") if s]
        # 形态：/new-password/<token> 或 /password/reset/<token>
        for i, s in enumerate(segs):
            if s in ("new-password", "reset-password", "password-reset"):
                if i + 1 < len(segs):
                    token = segs[i + 1]
                    break
    if not token:
        from core.otp_utils import _mask_token
        raise RuntimeError(f"重置链接缺少 token/code: {_mask_token(reset_url)}")
    origin = f"{parsed.scheme}://{parsed.netloc}"
    return origin, token


def submit_new_password(session: BrowserSession, reset_url: str, password: str) -> bool:
    """
    从重置链接建立重置态 cookie，POST /api/accounts/password/reset 提交新密码。

    逆向确认：/password/reset 的 body 只有 {"password": ...}，重置凭证由访问
    重置链接时服务端 Set-Cookie 的会话 cookie 承担（credentials include），
    token 不随 body/query 传。旧实现把 token 塞进 new-password body 是错的。
    请求与响应全文落日志供实弹校准。HTTP >= 400 抛 RuntimeError。
    """
    origin, token = _parse_reset_token(reset_url)

    # 访问重置链接：让服务端 set 重置态 cookie。follow redirect 保留 Set-Cookie，
    # 若链接 302 到 auth.openai.com 也会落在同一 cookie jar。token 解析失败不阻断
    # （仅用于日志脱敏展示），链接本身仍可访问。
    try:
        nav_headers = session.get_auth_navigate_headers(referer="https://auth.openai.com/")
        get_resp = session.get(reset_url, headers=nav_headers, allow_redirects=True)
        logger.info(
            "[2FA] [PWD] 访问重置链接: HTTP %s 落点=%s",
            get_resp.status_code, _mask_reset_url(get_resp.url or reset_url),
        )
        if get_resp.status_code >= 400:
            raise RuntimeError(f"访问重置链接失败: HTTP {get_resp.status_code}")
    except Exception as exc:
        raise RuntimeError(f"访问重置链接失败（链接可能已过期）: {str(exc)[:200]}") from exc

    headers = session.get_auth_headers(referer=reset_url)
    headers["content-type"] = "application/json"
    body = json.dumps({"password": password})
    logger.info("[2FA] [PWD] password/reset POST (origin=%s token=%s…)", origin, token[:4])
    resp = session.post(_RESET_SUBMIT_URL, headers=headers, data=body)
    logger.info(
        "[2FA] [PWD] password/reset 响应: HTTP %s body=%s",
        resp.status_code, (resp.text or "")[:400],
    )
    if resp.status_code >= 400:
        raise RuntimeError(f"password/reset 失败: HTTP {resp.status_code} {(resp.text or '')[:240]}")
    return True


def _mask_reset_url(url: str) -> str:
    """脱敏重置链接：token 类 query 值只留前 4 位，避免完整凭证进日志。"""
    try:
        from core.otp_utils import _mask_token
        return _mask_token(url)
    except Exception:
        return f"<url:{len(url)}chars>"
