...
def _agent_request_settings() -> tuple[float, int, float]:
    ...
    attempts = _int_setting("PROXY_RETRY_MAX_ATTEMPTS", 4, 1, 4)
    retry_delay = _float_setting("PROXY_RETRY_DELAY", 1.5, 0.0, 30.0)
...
def _retryable_agent_error(exc: Exception) -> bool:
    ...
    if any(marker in text for marker in (
        "invalid token", "invalid_token", "unauthorized", "status=401",
        "http 401", "account_deactivated", "account_deleted", "account_banned",
    )):
        return False
    return (
        "429" in text
...
def _run_generate(*, account_id: int, email: str, access_token: str, trigger: str, verify_task: bool) -> dict:
    ...
    try:
        ...
        routes = _plan_check_routes(None, attempts)
        attempts = min(attempts, len(routes))
        last_exc: Exception | None = None
        auth_json = None
        for attempt, route in enumerate(routes[:attempts], start=1):
