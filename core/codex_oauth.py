...
_NET_MAX_ATTEMPTS = max(1, min(4, int(_PROXY_RETRY_MAX_ATTEMPTS or 4)))
    ...
    try:
        ...
        for attempt in range(1, max_retries + 1):
            ...
            try:
                ...
                # 通知平台短信已发出（status=1）。BAD_STATUS=验证码已提前到达，非致命，直接轮询。
...
def save_codex_credential(storage: dict, email: str, plan_type: str) -> Path:
    ...
    if email and not str(payload.get("email") or "").strip():
