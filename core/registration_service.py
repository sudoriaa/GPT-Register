...
def set_run_deadline(minutes: float) -> None:
    """为本线程设置单次 run 的截止时间（秒级 time.time + minutes*60）。

    配合代理 -t-5 粘性：超过截止时间后，注册流程检查点（_check_manual_stop）会抛错，
    让重试机制换新 sid 重跑，而不是让一个 run 无限拖长到 IP 轮换。
    """
    deadline = None
    try:
        ...
        deadline = None
    _THREAD_CTX.run_deadline = deadline


def run_deadline_reached() -> bool:
    """当前线程是否已超过 set_run_deadline 设置的截止时间（未设置恒 False）。"""
    deadline = getattr(_THREAD_CTX, "run_deadline", None)
    if deadline is None:
        return False
    return time.time() >= deadline


def _run_max_minutes() -> float:
    from config import register as _r

    try:
        return float(getattr(_r, "RUN_MAX_MINUTES", 0) or 0)
    except (TypeError, ValueError):
        return 0.0


def _run_stagger_seconds() -> float:
    from config import register as _r

    try:
        return float(getattr(_r, "BATCH_STAGGER", 0) or 0)
    except (TypeError, ValueError):
        return 0.0
...
def _release_unconsumed_job_email(email: str | None, reason: str) -> None:
    """任务失败/停止兜底：回收尚未生成账号、仍处于 used 的邮箱领取。

    邮箱没注册成功就回 available（可重试），不标 failed 锁死。
    """
    if not email:
        return
    try:
        ...
        release_email_if_unconsumed(email, note=f"任务未消耗，已回可用可重试: {reason[:180]}")
    except Exception:
...
def _disable_job_email(email: str | None, reason: str) -> bool:
    """任务失败后回收邮箱：未注册成功（未生成账号）就回 available 可重试。

    不再用 disabled 锁死邮箱；已建成账号时 release_email_if_unconsumed 会自动跳过。
    """
    if not email:
        return False
    try:
        ...
        changed = release_email_if_unconsumed(email, note=f"注册失败自动回可用: {reason[:180]}")
        if changed:
            logger.warning("[Service] 注册失败邮箱已回可用: email=%s reason=%s", email, reason[:220])
        return changed
    except Exception:
        logger.exception("[Service] 回收邮箱失败: %s", email)
        return False
...
def _run_one_job(job_id: int, log_file: str, start_delay: float = 0.0) -> None:
    """单任务入口（线程池里跑这个）。

    start_delay: 并发模式下打破"同秒启动"批量信号的错峰间隔（秒）。睡眠发生在
    _activate_job 之前，期间 job 仍是 pending，用户可照常取消排队。
    """
    ...
    try:
        with _JobLogContext(log_file):
            ...
            registration_country = str(current.get("registration_country") or "").strip().upper()
            if registration_country:
                log_logger.info(
                    "[Job %s] 本次注册目标国家=%s；将在提交邮箱前实测代理出口",
                    job_id,
...
def submit_registration(
    ...
    with _executor_lock:
        ...
        # 并发 >1 时按序错峰：第 i 个 job 启动前等待 i*BATCH_STAGGER。
        # 串行(workers=1)自然排队，不额外等待。
        stagger = _run_stagger_seconds()
        jobs = []
        for i in range(count):
            if registration_country:
                ...
                # Preserve compatibility with existing DB adapters/mocks while
                # old jobs naturally interpret a missing field as no filter.
                job = db.create_job(email_source=email_source)
            start_delay = (i * stagger) if (effective_workers > 1 and stagger > 0) else 0.0
            try:
                executor.submit(_run_one_job, job["id"], job["log_file"], start_delay)
            except Exception as exc:
                db.update_job(
                    int(job["id"]),
                    status="failed",
...
def get_retry_info(job: dict) -> dict:
    ...
    if account:
        ...
        if codex_status == "success":
            ...
            return info
        # 已注册成功但 Codex 未完成：未开启 Codex 时不自动补跑，避免用户困惑
        #（ENABLE_CODEX_AUTO 只控制注册后自动授权；手动补跑应先去配置页开启 Codex）
        try:
            from config import codex as _codex_cfg
            codex_enabled = bool(getattr(_codex_cfg, "ENABLE_CODEX_AUTO", False))
        except Exception:
            codex_enabled = True
        if not codex_enabled:
            info["retryable"] = False
            info["retry_reason"] = "账号已注册成功；Codex 未开启（ENABLE_CODEX_AUTO=False），如需补跑请先在配置页开启 Codex 并配置 CPA"
            return info
        info.update({
            "retryable": True,
            "retry_action": "codex",
...
def _find_job_log_file(job_id: int, marker: str) -> Path | None:
    """按日志目录里 `[Job <id>]` 标记定位真实日志文件。

    任务记录的 log_file 偶尔会因数据文件并发写入而与实际写入的文件脱钩
    （WebUI 里表现为“暂无日志”）。这里兜底：扫描最近修改的日志文件头部，
    命中任务标记的第一个文件即为该任务真实日志。
    """
    try:
        logs_dir = Path(db.storage_paths()["logs_dir"])
    except Exception:
        return None
...
def read_job_log(job_id: int, max_bytes: int = 50_000) -> str:
    ...
    p = Path(job["log_file"])
    if not p.exists():
        # 记录路径失效时，用 [Job N] 标记从日志目录找回真实文件。
        p = _find_job_log_file(job_id, f"[Job {job_id}]")
        if p is None:
            return ""
    size = p.stat().st_size
    with p.open("rb") as f:
