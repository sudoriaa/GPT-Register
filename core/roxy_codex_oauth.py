...
from core.roxy_registration import (
    ...
    _email_otp_snapshot_is_verification,
...
_CODEX_LOGIN_STAGE_ATTEMPTS = 3
_CODEX_CALLBACK_STAGE_ATTEMPTS = 3
...
def _codex_recovery_abort(exc: BaseException) -> bool:
    """账号终态和用户停止信号不进入页面恢复链。"""
    text = str(exc or "")
    return (
        isinstance(exc, AccountUnusableError)
        or type(exc).__name__ in {"CodexRetryStopped", "StopRequested"}
        or "手动停止" in text
    )


def _login_stage_completed(driver) -> bool:
    """只读确认登录阶段是否已经推进，避免异常发生在跳转后又重复提交。"""
    if _extract_callback_url_from_any_window(driver):
        return True
    try:
    ...
    try:
        ...
        buttons = " ".join(str(item.get("text") or "") for item in (snapshot.get("buttons") or [])).lower()
        if any(token in buttons for token in ("allow", "authorize", "use workspace", "允许", "授权")):
            return True
    except Exception:
        pass
    return False
...
def _login_credential_stage_present(driver) -> bool:
    """邮箱提交动作报错时，确认页面是否已实际进入密码/验证码节点。"""
    try:
        return bool(_is_login_password_page(driver) or _is_email_verification_page(driver))
    except Exception:
        return False


def _lookup_account_credentials(email: str) -> dict:
    """按邮箱读取本地账号库的密码与 TOTP 密钥，供补跑场景账号密码+2FA 登录使用。

    密码优先级：chatgpt_password > password > extra_json.registration_password。
    """
    try:
        from core import db
        acc = db.get_account_by_email(email) or {}
    except Exception as exc:
        logger.debug("[Codex][Browser] 读取账号凭据失败：%s", str(exc)[:120])
        acc = {}
    ...
    if not password:
        try:
            ...
            extra = _json.loads(str(acc.get("extra_json") or "{}"))
...
def _current_totp_code(secret: str) -> str:
    """根据账号 TOTP 密钥生成当前 6 位验证码。"""
    try:
        import pyotp
        return str(pyotp.TOTP(secret).now())
    except Exception as exc:
        raise RuntimeError(f"生成 TOTP 验证码失败（请检查账号 totp_secret）：{exc}") from exc


def _is_code_entry_page(driver) -> bool:
    """验证码输入页（邮箱 OTP / 2FA-TOTP 共用的数字输入框页），排除登录密码页。"""
    try:
...
def _submit_login_password(driver, password: str) -> bool:
    """在 /log-in/password 页填入账号密码并提交。返回是否找到密码框并提交成功。"""
    ...
    if not result.get("ok"):
        logger.warning("[Codex][Browser] 登录密码页处理失败：%s", result)
        return False
    _human_type_text(driver, result.get("input"), password, clear=True)
    human_delay("form")
    _human_click(driver, result.get("button"), label="login_password_submit")
    logger.info("[Codex][Browser] 已填入账号密码并提交")
    return True


def _fill_totp_and_wait(driver, totp_secret: str, max_attempts: int = 2) -> bool:
    """在 2FA/TOTP 验证页填入当前验证码并等待离开验证页。TOTP 每 30s 换码，失败自动重试。"""
    for attempt in range(1, max_attempts + 1):
        if not _is_code_entry_page(driver):
            return True
        code = _current_totp_code(totp_secret)
        logger.info("[Codex][Browser] 填写 2FA(TOTP) 验证码：第 %s/%s 次", attempt, max_attempts)
        ...
        wait_end = time.time() + 45
        while time.time() < wait_end:
            if not _is_code_entry_page(driver):
                logger.info("[Codex][Browser] 2FA(TOTP) 验证通过，已离开验证页")
                return True
            if _is_login_password_page(driver):
                logger.warning("[Codex][Browser] 2FA 提交后回到登录密码页（密码错误？）")
                return False
            time.sleep(0.5)
        logger.warning("[Codex][Browser] 2FA(TOTP) 第 %s 次验证未通过/超时，生成新码重试", attempt)
    return False


def _wait_password_submit_result(driver, email: str, totp_secret: str, timeout: int = 60) -> str:
    """等待密码提交后的结果：处理 2FA(TOTP) 验证页，返回 'done' / 'email_otp'。"""
    ...
    while time.time() < end:
        if _is_login_password_page(driver):
            ...
            if any(x in text for x in (
                "incorrect", "wrong password", "invalid password", "invalid email or password",
                "密码不正确", "密码错误", "邮箱或密码", "パスワードが正しくありません", "password is incorrect",
            )):
                raise RuntimeError(f"账号密码错误，无法登录：email={email}")
            time.sleep(0.5)
            continue
        if _is_email_verification_page(driver):
            url = str(driver.current_url or "").lower()
            if "email-verification" in url or not totp_secret:
                return "email_otp"
            if _fill_totp_and_wait(driver, totp_secret):
                logger.info("[Codex][Browser] 账号密码 + 2FA 登录成功：%s", email)
                return "done"
            raise RuntimeError(f"2FA(TOTP) 验证码连续错误/过期，登录失败：email={email}")
        url = str(driver.current_url or "").lower()
        ...
        if _has_access_token(driver):
            return "done"
        time.sleep(0.5)
    raise RuntimeError(f"密码提交后长时间未离开登录密码页：email={email} state={_password_page_state(driver)}")


def _login_with_password_and_2fa(driver, email: str, password: str, totp_secret: str, timeout: int = 45) -> str:
    """账号密码 + 2FA(TOTP) 登录。

    补跑场景：提交邮箱后进入 /log-in/password 且账号已在本地保存密码时调用，
    直接填写账号密码登录，代替等待邮箱 OTP；若账号开启 2FA（totp_secret），
    在密码后的验证页自动填入生成的 TOTP 验证码。

    返回：
      - "done"：已完成登录，页面进入 consent/手机验证/授权/callback；
      - "email_otp"：明确进入邮箱验证码页，交回邮箱 OTP 流程。
    """
    transition_state = _wait_email_submit_next_state(
        driver,
        email,
        timeout=min(30, max(1, int(timeout or 45))),
    )
    logger.info(
        "[Codex][Browser] 邮箱提交后的登录转场状态：%s url=%s",
        transition_state,
        str(getattr(driver, "current_url", "") or "")[:180],
    )
    if transition_state in {"email_page", "unknown"}:
        raise RuntimeError(
            f"邮箱提交后页面未推进到密码/验证阶段：state={transition_state} "
            f"url={str(getattr(driver, 'current_url', '') or '')[:220]}"
        )
    if transition_state == "password":
        raise RuntimeError(
            f"邮箱提交后进入注册密码页而非登录密码页："
            f"url={str(getattr(driver, 'current_url', '') or '')[:220]}"
        )
    ...
    while time.time() < end:
        ...
        if not _is_login_password_page(driver):
            ...
            time.sleep(0.5)
            continue
        if not _submit_login_password(driver, password):
            raise RuntimeError(f"登录密码页未找到密码输入框：email={email} state={_password_page_state(driver)}")
        logger.info("[Codex][Browser] 已提交账号密码，等待密码验证结果：%s", email)
        return _wait_password_submit_result(driver, email, totp_secret)
    raise RuntimeError(
        f"账号密码+2FA 登录阶段超时：email={email} state={_password_page_state(driver)}"
    )
...
def _fill_email_and_otp_once(driver, email: str, otp_provider, auth_url: str,
                             password: str | None = None, totp_secret: str | None = None) -> None:
    ...
    # 注意：try 只包住“找邮箱输入框并提交”这一步；密码/2FA 登录的异常必须向上抛，
    # 不能被当作“未检测到邮箱输入框”吞掉。
    try:
        ...
        if _login_stage_completed(driver):
            logger.info("[Codex][Browser] 邮箱动作报错后只读探测确认页面已推进，跳过重复提交：%s", str(exc)[:120])
            return
        if _login_credential_stage_present(driver):
            logger.info(
                "[Codex][Browser] 邮箱提交动作报错，但页面已进入密码/验证码节点，继续当前节点：%s",
                str(exc)[:160],
            )
        else:
            raise RuntimeError(
                f"授权入口邮箱节点未完成：{type(exc).__name__}: {str(exc)[:220]} "
                f"url={str(getattr(driver, 'current_url', '') or '')[:220]}"
            ) from exc

    if password:
        # 补跑场景：账号已在本地保存密码（可能开启 2FA），直接用账号密码+2FA 登录，
        # 不再等待邮箱 OTP（已注册账号不会给密码登录邮箱发邮箱验证码）。
        outcome = _login_with_password_and_2fa(driver, email, password, totp_secret or "", timeout=45)
        if outcome == "done":
            logger.info("[Codex][Browser] 账号密码+2FA 登录完成，跳过邮箱 OTP：%s", email)
            return
        logger.info("[Codex][Browser] 密码登录未完成（%s），回退邮箱 OTP 流程", outcome)
    ...
    def _restart_email_otp_flow(reason: str) -> bool:
        ...
        账号已保存密码时优先重试密码+2FA 登录；成功返回 True，调用方结束整个流程。
        """
        ...
        try:
            ...
            if password:
                outcome = _login_with_password_and_2fa(driver, email, password, totp_secret or "", timeout=30)
                if outcome == "done":
                    logger.info("[Codex][Browser] 重启后账号密码+2FA 登录完成，跳过邮箱 OTP：%s", email)
                    return True
            else:
                _maybe_click_passwordless_after_email(driver, email, timeout=12)
        except Exception as exc:
    ...
    for otp_attempt in range(1, max_otp_attempts + 1):
        ...
        try:
            ...
            if _restart_email_otp_flow("等待验证码超时，避免点击 resend 导致 500"):
                return
            continue
        ...
        if _restart_email_otp_flow("验证码错误/过期或页面未跳转，避免点击 resend 导致 500"):
            return


def _fill_email_and_otp(driver, email: str, otp_provider, auth_url: str,
                        password: str | None = None, totp_secret: str | None = None,
                        max_attempts: int = _CODEX_LOGIN_STAGE_ATTEMPTS) -> None:
    """登录节点恢复器：失败时回到授权入口，重放邮箱 -> 密码 -> 2FA/邮箱 OTP。"""
    attempts = max(1, int(max_attempts or 1))
    history: list[str] = []
    last_exc: BaseException | None = None
    for attempt in range(1, attempts + 1):
        try:
            logger.info("[Codex][Browser][恢复][登录] 开始第 %s/%s 轮", attempt, attempts)
            _fill_email_and_otp_once(
                driver,
                email,
                otp_provider,
                auth_url,
            ...
            if _codex_recovery_abort(exc):
                raise
            last_exc = exc
            history.append(f"{attempt}:{type(exc).__name__}:{str(exc)[:240]}")
            if _login_stage_completed(driver):
                logger.info(
                    "[Codex][Browser][恢复][登录] 第 %s 轮异常后确认已进入后续节点，停止重放：%s",
                    attempt,
                    str(getattr(driver, "current_url", "") or "")[:180],
                )
                return
            if attempt >= attempts:
                break
            logger.warning(
                "[Codex][Browser][恢复][登录] 第 %s/%s 轮失败，回退授权入口重试邮箱/密码/2FA：%s: %s",
                attempt,
                attempts,
                type(exc).__name__,
                str(exc)[:220],
    ...
    raise RuntimeError(
        f"Codex 登录节点连续 {attempts} 轮失败，已回退授权入口重试；history={history}"
    ) from last_exc
...
def _finish_consent_workspace(driver, timeout: int | None = None) -> str:
    """点击 Codex consent/workspace 页面里的继续/允许按钮，直到 callback。"""
    end = time.time() + int(timeout or _roxy_cfg.ROXY_CODEX_CALLBACK_TIMEOUT)
    while time.time() < end:
        callback = _extract_callback_url_from_any_window(driver)
        if callback:
            return callback
...
def _finish_consent_workspace_with_recovery(
    driver,
    *,
    auth_url: str,
    resume_previous,
    max_attempts: int = _CODEX_CALLBACK_STAGE_ATTEMPTS,
) -> str:
    """callback 节点恢复器；先探测既有 callback，再刷新，最后回退到授权入口。"""
    attempts = max(1, int(max_attempts or 1))
    history: list[str] = []
    last_exc: BaseException | None = None
    for attempt in range(1, attempts + 1):
        ...
        if callback:
            return callback
        try:
            logger.info("[Codex][Browser][恢复][callback] 开始第 %s/%s 轮", attempt, attempts)
            return _finish_consent_workspace(driver)
        except Exception as exc:
            if _codex_recovery_abort(exc):
                raise
            last_exc = exc
            history.append(f"{attempt}:{type(exc).__name__}:{str(exc)[:240]}")
            # 点击 consent 后的导航异常最常见。先确认 callback 已存在，避免重复授权。
            callback = _extract_callback_url_from_any_window(driver)
            if callback:
                logger.info("[Codex][Browser][恢复][callback] 异常后已捕获 callback，跳过恢复动作")
                return callback
            if attempt >= attempts:
                break
            ...
            if attempt == 1:
                logger.warning(
                    "[Codex][Browser][恢复][callback] 捕获失败，先刷新当前 consent/workspace 节点：%s: %s",
                    type(exc).__name__,
                    str(exc)[:220],
                )
                try:
                    ...
                    if _codex_recovery_abort(recovery_exc):
                        raise
                    history.append(f"refresh:{type(recovery_exc).__name__}:{str(recovery_exc)[:200]}")

            logger.warning(
                "[Codex][Browser][恢复][callback] 当前节点恢复未成功，回退授权入口并重放最近安全节点：%s",
                str(auth_url or "")[:180],
            )
            try:
                resume_previous()
                ...
                if _codex_recovery_abort(recovery_exc):
                    raise
                history.append(f"resume:{type(recovery_exc).__name__}:{str(recovery_exc)[:220]}")
                logger.warning(
                    "[Codex][Browser][恢复][callback] 回退授权入口未完整推进，保留当前页面继续最后一轮探测：%s: %s",
                    type(recovery_exc).__name__,
                    str(recovery_exc)[:220],
                )

    raise RuntimeError(
        f"Codex callback 节点连续 {attempts} 轮失败，已尝试刷新和回退授权入口；history={history}"
    ) from last_exc
...
def _run_roxy_codex_oauth_once(
    ...
    try:
        ...
        # 补跑场景：账号已在本地保存密码/2FA 密钥，用账号密码+2FA 登录，不依赖邮箱 OTP。
        creds = _lookup_account_credentials(email)
        if creds.get("password"):
            logger.info(
                "[Codex][Browser] 账号已保存密码与 2FA 密钥，将使用账号密码+2FA 登录：email=%s password=yes totp=%s",
                email, "yes" if creds.get("totp_secret") else "no",
            )
        def _resume_before_callback() -> None:
            _fill_email_and_otp(
                driver, email, otp_provider, auth_url,
        ...
        _resume_before_callback()
        logger.info("[Codex][Browser] 手机验证处理完成/无需处理，等待授权确认和 callback")
        callback_url = _finish_consent_workspace_with_recovery(
            driver,
            auth_url=auth_url,
            resume_previous=_resume_before_callback,
        )
