...
class SkyvernClient:
    ...
    def create_browser_session(self) -> dict[str, Any]:
        ...
        configured_location = (
            self.proxy_country_code
            if self.proxy_country_code is not None
            else getattr(_cfg, "SKYVERN_PROXY_LOCATION", "")
        )
        proxy_location = self._normalize_proxy_location(str(configured_location or ""))
        if proxy_location:
    ...
    def open_session(self) -> SkyvernSession:
        ...
        try:
            ...
            # create 已成功但地址轮询/解析失败时，调用方拿不到 session 对象，
            # 因此由客户端在抛错前回收，避免预检轮换遗留远端会话。
            if session_id:
                try:
                    self.close_browser_session(session_id)
                    logger.info("[Skyvern] 创建后未能连接，已回收 browser session：%s", session_id)
                except Exception as close_exc:
                    logger.warning(
                        "[Skyvern] 创建失败后的 session 回收失败：%s: %s",
