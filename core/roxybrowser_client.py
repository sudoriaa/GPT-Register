...
class RoxyBrowserClient:
    def __init__(
        ...
        *,
        ...
        # A client instance belongs to one registration task. During duplicate-
        # IP rotation, prefer proxy modules this task has not tried yet.
        self._used_roxy_proxy_module_ids: set[str] = set()
    ...
    def request(self, method: str, path: str, *, params: dict | None = None, json_body: dict | None = None) -> dict:
        ...
        # 例外：Roxy 云端创建是异步的，高峰期返回「正在创建中，请稍等」——这是暂时状态，
        #       等待后重试 create 才是正确姿势，直接判失败会白白浪费整个注册任务。
        is_create = str(path or "").rstrip("/").endswith("/create") or "browser/create" in str(path or "")
        # Proxy/network operations share a hard four-attempt ceiling.  Keep a
        # smaller configured value when desired, but do not let a stale larger
        # value create an unexpectedly long retry loop.
        base_retries = min(4, max(1, int(getattr(_cfg, "ROXY_API_RETRIES", 4) or 4)))
        ...
        while True:
            ...
            try:
                logger.debug(
                    ...
                    method, url, params, json_body, attempt, base_retries,
                )
                ...
                if attempt > 1:
                    logger.info("[Roxy] API 重试成功：%s %s attempt=%s", method_u, path, attempt)
                return payload if isinstance(payload, dict) else {"data": payload}
            except Exception as exc:
                last_exc = exc
                creating = "正在创建" in str(exc) or "creating" in str(exc).lower()
                # Roxy 云端 /browser/create 是异步任务，高峰期服务端处理本身可能超时，
                # 在 payload.msg 里返回 "timeout of 15000ms exceeded"（注意：这是 Roxy
                # 服务端消息，不是客户端请求超时）。它与「正在创建中」一样属于暂时状态，
                # 等待后重试通常能成功，否则一次超时就废掉整个账号任务。
                # 潜在代价：若首次其实已建成，重试会多出一个孤儿环境（Roxy 面板可手动清理，
                # 运行成功后本流程也会删除环境），远小于整批失败的成本。
                server_timeout = "timeout" in str(exc).lower() and "ms" in str(exc).lower()
                retryable = self._is_retryable_error(exc)
                create_transient = creating or server_timeout
                # create 默认只允许 1 次；但「正在创建中」/云端异步超时是暂时状态，最多重试 base_retries 次
                effective_max = base_retries if (not is_create) or create_transient else 1
                if attempt >= effective_max or not (retryable or create_transient):
                    raise
                delay = base_delay * attempt
                logger.warning(
                    "[Roxy] API 请求失败，将在 %.1fs 后重试：%s %s attempt=%s/%s error=%s",
                    delay, method_u, path, attempt, effective_max, exc,
                )
                time.sleep(delay)
    ...
    def create_profile(self, payload: dict | None = None) -> str:
        ...
        # 代理来源：优先用 RoxyBrowser 应用内配置好的代理池（choose 模式，按 moduleId 引用）；
        # 未启用/取不到时回退到项目 config/proxy.py 的 PROXY_POOL（custom 模式，显式写代理）。
        use_roxy_pool = bool(getattr(_cfg, "ROXY_CREATE_USE_ROXY_PROXY_POOL", False))
        use_local_pool = bool(getattr(_cfg, "ROXY_CREATE_USE_PROXY_POOL", False))
        # An explicitly supplied route is the first attempt.  After it has
        # failed, subsequent profile attempts continue with configured pools.
        # An explicit empty string retains the established "direct" meaning.
        explicit_direct = self._preferred_proxy_supplied and not self._preferred_proxy_url
        if (
            not body.get("proxyInfo")
            and self._preferred_proxy_url
        ...
        if not body.get("proxyInfo") and not explicit_direct and (use_roxy_pool or use_local_pool):
            ...
            if not body.get("proxyInfo") and use_local_pool:
                ...
                proxy_url = self._pick_local_proxy_url(_proxy_cfg)
                if proxy_url:
    ...
    def _pick_local_proxy_url(self, proxy_cfg=None) -> str:
        """Pick a different project proxy for each profile attempt.

        The selection wraps only after every unique configured route has been
        tried.  This preserves useful retries even when the pool contains just
        one entry while preventing a random re-pick of a known-bad endpoint.
        """
        ...
        if not pool:
            try:
                ...
                selected = ""
            if selected:
                self._used_local_proxy_urls.add(selected)
            return selected
        unused = [value for value in pool if value not in self._used_local_proxy_urls]
        # Keep the configured picker as the first choice (and therefore retain
        # compatibility with callers/tests that inject a deterministic picker),
        # but reject it when this task already tried that route.
        try:
            picked_raw = proxy_cfg.pick_proxy()
            picked = str(picked_raw or "").strip() if isinstance(picked_raw, str) else ""
        except Exception:
            picked = ""
        if picked and picked not in self._used_local_proxy_urls:
            selected = picked
        else:
            selected = random.choice(unused or pool)
        self._used_local_proxy_urls.add(selected)
        return selected

    def fetch_proxy_pool(self, workspace_id: str | int | None = None) -> list[dict]:
        """
        获取 RoxyBrowser 应用内配置好的代理池（GET /proxy/list）。

        返回代理池里每条代理的原始字段，例如
            id / checkStatus / protocol / ipType / host / port /
            proxyUserName / proxyPassword / refreshUrl / remark / ...
        id 是代理池条目 id：创建环境时以 choose 模式（proxyInfo.moduleId）引用该代理，
        Roxy 侧按 moduleId 读取代理，本项目不接触代理账号密码。
        """
        wid = _workspace_id_value() if workspace_id is None else workspace_id
        if not wid:
            raise RuntimeError("获取 Roxy 代理池需要 workspaceId，请配置 ROXY_WORKSPACE_ID")
        payload = self.request("GET", "/proxy/list", params={"workspaceId": wid})
        data = payload.get("data") or {}
        rows = data.get("rows") or []
        return [r for r in rows if isinstance(r, dict)]

    def _pick_roxy_pool_proxy_info(self) -> dict:
        """从 Roxy 应用内代理池随机取一条有效代理，返回 choose 模式的 proxyInfo。

        Roxy /browser/create 里 proxyInfo.moduleId 存在且非 0 时，
        会自动把 proxyMethod 设为 "choose"，即直接引用代理池里该代理。
        优先选 checkStatus=1（检测通过）的代理，取不到时回退任意一条。
        """
        try:
            proxies = self.fetch_proxy_pool()
        except Exception as exc:
            logger.warning("[Roxy] 获取 Roxy 代理池失败：%s", exc)
            return {}
        if not proxies:
            logger.warning("[Roxy] 已启用 ROXY_CREATE_USE_ROXY_PROXY_POOL，但 Roxy 代理池为空，本次创建环境不设置代理")
            return {}
        valid = [p for p in proxies if p.get("checkStatus") == 1]
        candidates = valid or proxies
        unused = [
            p for p in candidates
            if str(p.get("id") or "").strip() not in self._used_roxy_proxy_module_ids
        ]
        proxy = random.choice(unused or candidates)
        module_id = proxy.get("id")
        if module_id is None or str(module_id).strip() == "":
            logger.warning("[Roxy] Roxy 代理池条目缺少 id，本次创建环境不设置代理")
            return {}
    ...
    @staticmethod
    def _wait_debugger_ready(addr: str, timeout: float = 60.0) -> None:
        """轮询等待 Roxy 浏览器 debugger 端口可连接。

        open 接口返回时本地转发端口可能尚未监听，Selenium 立即连会报
        'session not created: cannot connect to chrome'。这里等端口就绪再返回。
        """
        import socket
        import time
        ...
        while time.time() - start < timeout:
            try:
                ...
                time.sleep(1)
        logger.warning("[Roxy] debugger 端口 %s 在 %.0fs 内未就绪，继续尝试连接", addr, timeout)
    ...
    def open_profile(self, profile_id: str | None = None) -> RoxyOpenResult:
        ...
        if debugger_address:
            # 端口可能尚未就绪，等可连接再返回，避免 Selenium 立即连接失败。
            self._wait_debugger_ready(debugger_address)
