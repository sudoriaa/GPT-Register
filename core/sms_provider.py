...
def _sms_api_key() -> str:
    ...
    if _provider() == "smsbower":
        return str(getattr(_cfg, "SMSBOWER_API_KEY", "") or "").strip() or str(getattr(_cfg, "SMS_API_KEY", "") or "").strip()
...
def _sms_api_base() -> str:
    ...
    if _provider() == "smsbower":
        return str(getattr(_cfg, "SMSBOWER_API_BASE", "") or "https://smsbower.page/stubs/handler_api.php").strip()
    return str(getattr(_cfg, "SMS_API_BASE", "") or "https://api.grizzlysms.com/stubs/handler_api.php").strip()


def _sms_service(default: str = "") -> str:
    """服务码：smsbower 用 SMSBOWER_SERVICE（默认 dr=OpenAI），否则用 SMS_SERVICE。"""
    if _provider() == "smsbower":
        return str(getattr(_cfg, "SMSBOWER_SERVICE", "") or "dr").strip() or default
    return str(getattr(_cfg, "SMS_SERVICE", "") or "openai").strip() or default


def _sms_provider_ids() -> str:
    """渠道号（providerIds，逗号分隔）。GrizzlySMS / SMSBower 都支持。"""
    return str(getattr(_cfg, "SMS_PROVIDER_IDS", "") or "").strip()
...
def _request_grizzly(http: CurlSession, params: dict) -> str:
    ...
    统一识别公共错误码并抛对应异常。
    """
    base_params = {"api_key": _sms_api_key()}
    base_params.update(params)
    resp = http.get(_sms_api_base(), params=base_params)
...
def acquire_number(
    ...
    try:
        ...
        params = {
            "action": "getNumber",
            "service": service or _sms_service(),
            "country": country or _cfg.SMS_COUNTRY,
        }
        if _cfg.SMS_MAX_PRICE:
            params["maxPrice"] = _cfg.SMS_MAX_PRICE
        provider_ids = _sms_provider_ids()
        if provider_ids:
...
def mark_sms_sent(activation_id: str, http: CurlSession | None = None) -> None:
    """标记"短信已发出"（setStatus=1），让平台进入等待收码状态。尽力而为，不抛异常。
