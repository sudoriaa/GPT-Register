...
class AuthFlow:
    ...
    def __init__(
        ...
        on_warmup_done: Optional[Any] = None,
    ):
        # warmup() 成功种 cookie 后、进入 CSRF/authorize 之前回调（签名 flow -> None）。
        # 本项目用它挂 anonymous_bootstrap 预热，让会话更接近真实 Web 首屏。
        ...
        # 可选注入：外部调用方（本项目 adapter）指定姓名/生日，create_account 用之；
        # 留空则保持原随机生成行为。
        ...
        # 会话级前端标识：真实 ChatGPT Web 前端每个会话有一个 oai-session-id（UUID）。
        # 协议侧补一个，让 chatgpt.com 请求更像真实前端（对试用资格/信任度有帮助）。
    ...
    def _rotate_impersonate_session(self) -> bool:
        ...
        self.session = create_http_session(
            ...
            impersonate=imp, user_agent=self._ua,
        )
        return True
    ...
    def _common_headers(self, referer: str = "https://chatgpt.com/") -> dict:
        ...
        # chatgpt.com 侧请求补 ChatGPT Web 前端上报头（真实前端每次 API 调用都带），
        # 提升会话“来自真实 Web 前端”的信任信号，对账号试用资格/存活有帮助。
        # 值可通过 env_overrides（OAI_CLIENT_BUILD_NUMBER / OAI_CLIENT_VERSION）注入。
        if "chatgpt.com" in host:
            device_id = (self.result.device_id or "").strip() or (self.session.cookies.get("oai-did", "") or "").strip()
            if device_id:
                headers["oai-device-id"] = device_id
            build = self._get_env("OAI_CLIENT_BUILD_NUMBER", "").strip()
            ver = self._get_env("OAI_CLIENT_VERSION", "").strip()
            if build:
                headers["oai-client-build-number"] = build
            if ver:
                headers["oai-client-version"] = ver
            lang = str(fp.get("lang_full") or "").strip()
            if lang:
                headers["oai-language"] = lang.split(",")[0].strip()
            headers["oai-session-id"] = self._oai_session_id
    ...
    def create_account(self) -> str:
        ...
        # 支持外部注入姓名/生日（本项目 adapter 可指定）；留空则随机生成。
        name = self._display_name or f"{random.choice(_FIRST)} {random.choice(_LAST)}"
        birthdate = self._birthdate or (
    ...
    def run_register(self, mail_provider: MailProvider) -> AuthResult:
        ...
        # warmup 种好 oai-did 后，先跑匿名态 ChatGPT 首屏预热（statsig/analytics/
        # 模型列表等），让会话更像真实 Web 用户，再进入 CSRF/authorize。可失败，不阻断。
        if self._on_warmup_done is not None:
            try:
                self._on_warmup_done(self)
            except Exception as _bexc:
                logger.warning("on_warmup_done 预热回调异常（不影响注册）: %s", _bexc)
        ...
        # ⚠️ passwordless_signup **也是新账号**：signup() 只返回一个 bool，把
        #    create_account_password / passwordless_signup / 已有账号压成了
        #    True / False / False 三种状态。passwordless_signup 必须排除在
        #    "已有账号 fast-fail"之外，否则新号全被误杀（source 靠 WEBUI_ALLOW_LOGIN=1
        #    才绕过这个问题，但那样真正已有的号会白等 90s）。这里按 mode 精确区分：
        _mode_lc = (self._existing_email_verification_mode or "").lower()
        is_pooled_existing = (
            not is_new
            and getattr(mail_provider, "pooled", False)
            and _mode_lc != "passwordless_signup"
        )
    ...
    # ── 对已注册但没 RT 的账号，重新绑手机拿 refresh_token ──
    def rebind_phone_get_refresh_token(self, *, email: str = "") -> str:
        """重新跑 Codex OAuth 拿 refresh_token，命中 add-phone 时走 SMS 接码绑号。

        前置：调用方先 from_existing_credentials() 恢复会话（session_token /
        access_token / device_id），可选 set self.result.email。
        返回 refresh_token；拿不到抛 RuntimeError。
        """
        if email:
            self.result.email = email
        # 确保 RT 交换开关开着（重绑就是冲着 RT 来的）
        if not self._env_flag("OAUTH_CODEX_RT_EXCHANGE", "1"):
            self._env_overrides["OAUTH_CODEX_RT_EXCHANGE"] = "1"
        ok = False
        try:
            ok = self.oauth_codex_rt_exchange()
        except Exception as e:
            logger.warning(f"[rebind] Codex OAuth 异常: {e}")
        rt = (self.result.refresh_token or "").strip()
        if not rt:
            raise RuntimeError("重新绑手机后仍未拿到 refresh_token")
        logger.info(f"[rebind] ✅ 拿到 refresh_token (len={len(rt)})")
        return rt
