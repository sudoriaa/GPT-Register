...
def parse_sms_routes(text: str) -> list[dict]:
    """解析「自选国家 + 对应渠道号」路由池。每行 `国家:渠道号1,渠道号2`（渠道号可选）。

    例：
        52:3170,4120     ← 泰国，只从渠道 3170/4120 拿号
        44:3309          ← 英国，只从渠道 3309
        1:               ← 美国，任意渠道
    返回 [{"country": "52", "provider_ids": "3170,4120", ...}, ...]
    """
    out: list[dict] = []
    for line in (text or "").splitlines():
        ...
        if not line or line.startswith("#"):
            continue
        country, _, prov = line.partition(":")
        country = (country or "").strip()
        if not country:
            continue
        out.append({
            "country": country,
            "provider_ids": str(prov or "").strip().strip(","),
        })
    return out
...
class SmsBowerProvider(BaseSmsProvider):
    ...
    def __init__(
        ...
        *,
        ...
        # 指定渠道号（运营商 ID，逗号分隔）。参考 smsbower 文档：
        #   getNumber 支持 providerIds（只买这些渠道）/ exceptProviderIds（排除）/ minPrice。
        self.provider_ids = str(provider_ids or "").strip().strip(",")
        self.except_provider_ids = str(except_provider_ids or "").strip().strip(",")
        # 禁用国家/渠道（自动选号时黑名单，如 12=美国虚拟卡收不到 SMS）
        self.blocked_countries = {
            str(c).strip() for c in str(blocked_countries or "").replace(";", ",").split(",")
            if str(c).strip()
        }
        # 自选「国家 + 渠道号」路由池：[{"country": "52", "provider_ids": "3170,4120"}, ...]
    ...
    def get_channels(self, service: Optional[str] = None, country: Optional[str] = None) -> list[dict]:
        """查指定国家的渠道（运营商）列表：getPricesV3 -> [{provider_id, count, price}, ...]。

        参考 smsbower 文档：getPricesV3 返回「国家 -> 服务 -> 渠道」层级，每个渠道对象含
            {"count": 542, "price": 0.12, "provider_id": 3170}
        递归展平，价格升序 —— 给 WebUI「查渠道」按钮用，方便选渠道号。
        """
        service_code = str(service or self.default_service or SMS_DEFAULT_SERVICE).strip()
        country_id = str(country or self.default_country or "").strip()
        if not country_id:
            raise RuntimeError("SmsBower 查渠道需要指定国家")
        data = self._request({
        ...
        def visit(value, key_hint=None):
            if isinstance(value, dict):
                if any(k in value for k in ("provider_id", "providerId")):
                    pid = str(value.get("provider_id", value.get("providerId")))
                    found[pid] = {
                        "provider_id": pid,
                        "count": _safe_int(value.get("count"), 0),
                        "price": _safe_float(value.get("price"), None),
                    }
                    return
                if key_hint is not None and "count" in value and "price" in value:
                    pid = str(key_hint)
                    found[pid] = {
                        "provider_id": pid,
                        "count": _safe_int(value.get("count"), 0),
        ...
        rows.sort(key=lambda r: (r["price"] is None, r["price"] or 0, r["provider_id"]))
        return rows
    ...
    def get_best_country(self, service: Optional[str] = None, *,
        ...
        blocked = blocked_countries or self.blocked_countries

        def _pick(stock_threshold: int) -> Optional[str]:
            for row in rows:
                cid = str(row.get("country") or "")
                # 黑名单国家/渠道：直接跳过（如美国虚拟卡收不到 OpenAI SMS）
                if blocked and cid in blocked:
                    continue
    ...
    def _load_cache(self, service: str, country: str) -> Optional[dict]:
        ...
        if any(str(cache.get(k) or "") != str(v) for k, v in identity.items()):
            return None
        # ⚠️ 并发保护：这个号正被别的 worker 用（in_use=True）→ 不复用，换新号
        if cache.get("in_use"):
            logger.debug("SmsBower 缓存号正在被使用(in_use)，跳过复用")
            return None
    ...
    def _request_number_single_action(self, action: str, service: str, country: str, *,
        ...
        渠道/价格参数可覆盖（自选路由池时每条路由带自己的渠道号）。
        """
        # 未显式传的用 provider 全局默认
        max_price = self.max_price if max_price is None else max_price
        min_price = self.min_price if min_price is None else min_price
        provider_ids = self.provider_ids if provider_ids is None else provider_ids
        except_provider_ids = self.except_provider_ids if except_provider_ids is None else except_provider_ids
        ...
        # 指定渠道号（providerIds）：参考 smsbower 文档，只从这些渠道/运营商拿号
        if provider_ids:
            common["providerIds"] = provider_ids
        if except_provider_ids:
            common["exceptProviderIds"] = except_provider_ids
        logger.info(
            "SmsBower %s: service=%s country=%s maxPrice=%s minPrice=%s providerIds=%s",
            action, service, country,
            common.get("maxPrice", "未设置"),
            common.get("minPrice", "未设置"),
            common.get("providerIds", "未指定"),
        )
    ...
    def get_number(self, *, service: str, country: str = "",
                    ...
                    routes: Optional[list[dict]] = None) -> SmsActivation:
        """租号。支持多国家候选依次尝试（按入参顺序）。

        country_candidates: 候选国家 ID 列表，按这个顺序依次尝试；空时只用 country 单个。
        routes: 自选「国家 + 渠道号」路由池（优先）。每条 {country, provider_ids}，
                只从这些 国家+渠道 组合里拿号；渠道号为空 = 该国家任意渠道。
        ...
        # 路由池优先：country_candidates 由路由池的国家推导
        route_map: dict[str, dict] = {}
        if routes:
            for r in routes:
                rc = str(r.get("country") or "").strip()
                if rc:
                    route_map[rc] = r
            route_countries = list(route_map.keys())
            if not country_candidates:
                country_candidates = route_countries
        # 单一 country 兜底
        if not country_candidates:
        ...
        with _SMS_VERIFY_LOCK:
            with _SMS_CACHE_LOCK:
                ...
                # 双重 for：外层国家(路由) × 内层 action（V2 / V1）
                failures: list[str] = []
                ...
                for cid in country_candidates:
                    ...
                    r_provider_ids = str(route.get("provider_ids") or "").strip() or None
                    for action in ("getNumberV2", "getNumber"):
                        try:
                            info = self._request_number_single_action(
                                action, service_code, cid,
                                provider_ids=r_provider_ids,
                            )
    ...
    def report_success(self, activation_id: str) -> bool:
        with _SMS_CACHE_LOCK:
            ...
            if cache and str(cache.get("activation_id")) == str(activation_id):
                ...
                cache["in_use"] = False   # 本号使用结束，恢复可复用
...
def create_sms_provider(provider_key: str, config: dict) -> BaseSmsProvider:
    ...
    min_price = _safe_float(config.get("sms_min_price"), -1)
    provider_ids = str(config.get("sms_provider_ids") or "").strip()
    except_provider_ids = str(config.get("sms_except_provider_ids") or "").strip()
    blocked_countries = str(config.get("sms_blocked_countries") or "").strip()
    routes = parse_sms_routes(config.get("sms_routes") or "")
...
class PhoneCallbackController:
    ...
    def get_phone(self) -> str:
        ...
        # 复用锁：只有开「号码复用」才需要（防两个 worker 并发抢同一个 cache 号）。
        # 拿到号就立刻释放 —— 等码阶段不占锁，否则高并发全串在等码上 = 假并发。
        if isinstance(provider, SmsBowerProvider) \
                and getattr(provider, "reuse_phone_to_max", False) \
                and not self._verify_lock_acquired:
            _SMS_VERIFY_LOCK.acquire()
            self._verify_lock_acquired = True

        # 收集候选国家列表：自选国家+渠道池 > 用户多选 > 自动选号 best > 单一 country
        ...
        # 禁用国家/渠道黑名单（如 12=美国虚拟卡收不到 OpenAI SMS）
        blocked_raw = str(self.config.get("sms_blocked_countries") or "").strip()
        blocked_set = {c.strip() for c in blocked_raw.replace(";", ",").split(",") if c.strip()}
        if blocked_set:
            allowed_list = [c for c in allowed_list if c not in blocked_set]
        ...
        if self.auto_select_country and isinstance(provider, SmsBowerProvider):
            # 自选「国家+渠道」路由池（最高优先级）：手机号只从这个池子里拿
            routes = parse_sms_routes(self.config.get("sms_routes") or "")
            if blocked_set:
                routes = [r for r in routes if r.get("country") not in blocked_set]
            if routes:
                self.log(f"🔍 自选国家+渠道池: {len(routes)} 条路由")
                country_candidates = [
                    str(r.get("country") or "").strip() for r in routes
                    if str(r.get("country") or "").strip()
                ]
                self.log(f"  候选: {','.join(country_candidates)}")
            elif allowed_list:
        ...
        # 号已拿到：立刻释放复用锁，让其他 worker 能并发租号/等码
        self._release_lock()
