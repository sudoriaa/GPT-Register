...
# 代理前置通道（pre-proxy / uplink）：
# 本项目所有走 PROXY_POOL / PLAN_CHECK_PROXY 的代理连接，都先经此前置代理再连目标代理。
# 用途：代理商的源 IP 白名单只放行指定出口（例如本机 Clash 的东京出口）时，
#       用它先绕到该出口再连住宅代理，从而绕过「直连源 IP 不在白名单 → 403」。
# 仅支持 socks5h:// / socks5://（libcurl 的 PRE_PROXY 只认 SOCKS）；Cloak 转发器同时支持 http://。
# 留空 = 不使用前置通道（默认，与旧行为一致）。
PROXY_PRE_PROXY: str = ""

# Cloak 创建浏览器窗口前先验证代理链可用；失败时立即尝试池内下一条代理。
# 探测复用浏览器 GeoIP 接口，只验证网络连通性和可解析的公网出口 IP。
PROXY_HEALTH_CHECK: bool = True
PROXY_HEALTH_TIMEOUT: float = 8.0

# 所有依赖代理的功能共用这个上限：一次请求失败后切换到下一个代理，
# 最多执行 4 次。业务层的凭据/账号错误不会因为这个配置被重复提交。
PROXY_RETRY_MAX_ATTEMPTS = 4
...
apply_env_overrides(globals(), {
    ...
    'PROXY_HEALTH_TIMEOUT': 'float',
    'PROXY_RETRY_MAX_ATTEMPTS': 'int',
