...
from config.email import (
    ...
    MAILCOM_SESSION_DIR,
    MAILCOM_PROXY,
    MAILCOM_REQUEST_TIMEOUT,
    MAILCOM_MESSAGE_LIMIT,
    MAILCOM_CHANGE_PASSWORD_BEFORE_REGISTER,
    GPTMAIL_API_KEY,
    MAIL_NEST_API_KEY,
...
from config.oaics import (
    ...
    OAICS_CURRENCY,
    OAICS_EXPECTED_METHOD,
    OAICS_PLAN_NAME,
    OAICS_PROMO_CAMPAIGN,
    OAICS_INCLUDE_PROMO,
)
...
def _refresh_top_level_constants() -> None:
    ...
    from config import browser, openai_protocol, proxy as _proxy, register, email, twofa, oaics, roxybrowser, cloakbrowser, browser_use, skyvern, codex, extract_link, sub2api, humanize, flow_trigger
    # 简单粗暴：枚举一遍重要常量，覆盖到 _self
    for src in (browser, openai_protocol, _proxy, register, email, twofa, oaics, roxybrowser, cloakbrowser, browser_use, skyvern, codex, extract_link, sub2api, humanize, flow_trigger):
        for k in dir(src):
...
__all__ = [
    ...
    # proxy
    "PROXY_POOL", "PROXY_HEALTH_CHECK", "PROXY_HEALTH_TIMEOUT", "PROXY_RETRY_MAX_ATTEMPTS", "PROXY_RETRY_DELAY", "PLAN_CHECK_PROXY_MODE", "PLAN_CHECK_PROXY",
    ...
    "MAILCOM_NODE_BIN", "MAILCOM_SESSION_DIR", "MAILCOM_PROXY", "MAILCOM_REQUEST_TIMEOUT",
    "MAILCOM_MESSAGE_LIMIT", "MAILCOM_CHANGE_PASSWORD_BEFORE_REGISTER",
    ...
    "ENABLE_2FA",
    # oaics
    "OAICS_AUTO_CHECK", "OAICS_TIMEOUT", "OAICS_MAX_ATTEMPTS",
