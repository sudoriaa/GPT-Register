...
def load_env(*, override: bool = False) -> Path:
    ...
    if _ENV_PATH.exists():
        try:
            ...
            # Ignore oversized individual values while retaining the rest of
            # the environment configuration on Windows.
            for key, value in read_env_file().items():
                if value is None or (not override and key in os.environ):
                    continue
                try:
                    os.environ[key] = value
                except ValueError:
