...
# 创建 Roxy 环境时是否使用 RoxyBrowser 应用内配置好的代理池（GET /proxy/list）：
#   False = 不主动使用 Roxy 代理池
#   True  = 每次创建环境时从 Roxy 应用内代理池随机取一条有效代理，按 moduleId 引用（choose 模式）
# 开启后优先于上方 ROXY_CREATE_USE_PROXY_POOL（项目 config/proxy.py 的 PROXY_POOL）。
# Roxy 侧按 moduleId 读取代理，本项目不接触代理账号密码。
ROXY_CREATE_USE_ROXY_PROXY_POOL: bool = False

# 注册前检查当前 Roxy 环境的真实出口 IP：若该 IP 已存在于历史注册账号，
# 或已被本进程内另一条并发注册占用，则关闭临时环境并换代理重试。
# 最多检查 5 个环境；第 5 个无论是否重复都继续注册，避免无限换 IP。
ROXY_AVOID_DUPLICATE_REGISTRATION_IP: bool = True
