...
# 检测到 Cloudflare Turnstile 人机验证时，自动点击 checkbox 并等待 token 生成，
# 避免验证卡住导致流程把窗口自动关闭。设为 False 可整体关闭。
CLOUD_FLARE_AUTO_SOLVE: bool = True

# 单次挑战处理的等待上限（秒）。无挑战时该逻辑几乎不耗时。
CLOUD_FLARE_SOLVE_TIMEOUT: int = 60

# ---- .env overrides for WebUI editable fields ----
apply_env_overrides(globals(), {'BROWSER_LOCALE_PROFILE': 'str', 'AUTO_BROWSER_LOCALE_FROM_IP': 'bool', 'IP_GEO_TIMEOUT': 'float', 'REJECT_CLOUD_PROXY': 'bool', 'CLOUD_FLARE_AUTO_SOLVE': 'bool', 'CLOUD_FLARE_SOLVE_TIMEOUT': 'int', 'IMPERSONATE': 'str'})
